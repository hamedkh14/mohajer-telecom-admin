...
const CustomerItem = ({ item, openMenu }: { item: any; openMenu: any }) => {
  const price = Number(item?.wallet_available_balance) - Number(item?.wallet_locked_balance)
  return (
    <View style={[styles.container, item?.isDelete && styles.deleted]}>
      <View style={styles.topContainer}>
        <View style={{ flexDirection: "row-reverse", gap: Spacing[1] }}>
          ...
          >
            ...
            >
              <ThemedText type="text">{item?.name}</ThemedText>
              {item?.status === false && (
                <LockClosedIcon color={Colors.bgWarning} size={Sizes.base} />
              )}
            </View>
      ...
      <Divider
        lineStyle={{ backgroundColor: Colors.whiteAlpha2 }}
        style={{ marginVertical: 0 }}
      />
      <View style={styles.bottomContainer}>
        <ThemedText type="caption">موجودی کیف پول</ThemedText>
        <Price
          type={ price < 0 ? "withdrawal" : price > 0 ? "deposit" : "text" }
          price={ price }
          style={{ height: 28 }}
        />
      </View>
