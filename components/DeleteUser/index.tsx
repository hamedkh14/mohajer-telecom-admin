...
const DeleteUser = ({open, onClose, item}: modalProp) => {
  ...
  const onDelete = () => {
    ...
    if(item) {
      ...
      .then((res) => {
        Toast.show({
          type: 'success',
          text1: 'عملیات حذف مشتری با موفقیت انجام شد!'
        })
      })
      .catch((err) => {
        Toast.show({
      ...
      <View style={styles.modalBackground}>
        <View style={styles.container}>
          <View style={styles.topContent}>
            <ThemedText type='text' style={{marginBottom: Spacing[4]}}>حذف مشتری</ThemedText>
            <ThemedText type='caption' style={{textAlign: 'center'}}>شما می خواهید مشتری {item?.name} را حذف کنید. آیا مطمئن هستید؟</ThemedText>
          </View>
