...
const DeleteService = ({open, onClose, item}: modalProp) => {
    const { mutateAsync: updateService } = useUpdateService(item?.id);
  ...
  const onDelete = () => {
    ...
    if(item) {
      updateService({isDelete: true})
      .then((res) => {
        Toast.show({
          type: 'success',
          text1: 'عملیات حذف بسته با موفقیت انجام شد!'
        })
      })
      .catch((err) => {
        Toast.show({
      ...
      <View style={styles.modalBackground}>
        <View style={styles.container}>
          <View style={styles.topContent}>
            <ThemedText type='text' style={{marginBottom: Spacing[4]}}>حذف بسته</ThemedText>
