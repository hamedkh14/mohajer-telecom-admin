...
const AlmasService = () => {
  ...
  const [filter, setFilter] = useState(`?filter=(type='almas'%26%26isDelete=False)`)
  const { 
    isLoading,
    data,
    isError,
  ...
  const onSearch = () => {
    if(
        (searchValue === '' && filter === `?filter=(type='almas'%26%26isDelete=false)`) || 
        (searchValue !== '' && filter === `?filter=(type='almas'%26%26isDelete=false%26%26title ~ '${searchValue}')`)
    ) {
      return
    }
    ...
    if(searchValue === '') {
      setFilter(`?filter=(type='almas'%26%26isDelete=false)`)
    }else {
      setFilter(`?filter=(type='almas'%26%26isDelete=false%26%26title ~ '${searchValue}')`)
    }
