...
const UcPubgService = () => {
  ...
  const [filter, setFilter] = useState(`?filter=(type='uc-pubg'%26%26isDelete=false)`)
  const { 
    isLoading,
    data,
    isError,
  ...
  const onSearch = () => {
    if(
        (searchValue === '' && filter === `?filter=(type='uc-pubg'%26%26isDelete=false)`) || 
        (searchValue !== '' && filter === `?filter=(type='uc-pubg'%26%26isDelete=false%26%26title ~ '${searchValue}')`)
    ) {
      return
    }
    ...
    if(searchValue === '') {
      setFilter(`?filter=(type='uc-pubg'%26%26isDelete=false)`)
    }else {
      setFilter(`?filter=(type='uc-pubg'%26%26isDelete=false%26%26title ~ '${searchValue}')`)
    }
