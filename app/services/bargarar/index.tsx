...
const BargararService = () => {
  ...
  const onSearch = () => {
    if(
        (searchValue === '' && filter === `?filter=(type='bargarar'%26%26isDelete=false)`) || 
        (searchValue !== '' && filter === `?filter=(type='bargarar'%26%26isDelete=false%26%26title ~ '${searchValue}')`)
    ) {
      return
    }
    ...
    if(searchValue === '') {
      setFilter(`?filter=(type='bargarar'%26%26isDelete=false)`)
    }else {
      setFilter(`?filter=(type='bargarar'%26%26isDelete=false%26%26title ~ '${searchValue}')`)
    }
...
export default BargararService